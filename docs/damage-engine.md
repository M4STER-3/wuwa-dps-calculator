# Damage Engine V0.1 — dégâts standards
...
Une action marquée `scaling: "tuneAmp"` n'est jamais transformée silencieusement en attaque ATK standard. Depuis Damage Engine V0.2, elle devient calculable uniquement avec un contexte Tune Rupture explicite; sans ce contexte, le résultat reste `unsupported` avec la raison `missing-tune-context`.
...
Les formules séparées Tune Break et Tune Rupture sont documentées dans [`docs/tune-damage-engine.md`](./tune-damage-engine.md).
