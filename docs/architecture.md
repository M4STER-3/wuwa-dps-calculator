...
`finalStats.tuneBreakBoost` suit la même règle anti-double-comptage : il contient la valeur permanente du panneau. Une ancienne Box sans ce champ est normalisée à la lecture (`10` pour Aemeath, `0` sinon) sans supprimer les builds. Les bonus temporaires de Tune Break Boost restent des paramètres explicites du contexte de combat.
...
Voir [`docs/tune-damage-engine.md`](./tune-damage-engine.md) pour les familles Tune Break et Tune Rupture du Damage Engine V0.2.
