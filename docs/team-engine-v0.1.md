...
## V0.3 correctness and Rotation Validation Lab

Direct Team actions now separate `action-start`, supported aggregate damage, `damage-dealt`, and exact hit context. Aggregate damage never fabricates `action-hit`; hit-driven statuses and coordinated responses require verified hit timing or an explicit external combat event. Failed before-action transactions abort atomically, action cooldowns are actor-local on the shared clock, expired target statuses are removed at the event boundary, and refreshable effect instances refresh rather than stack.

Runtime actors have `currentForm`, initialized only from verified `defaultForm` data or explicit actor input. Capability `forms` are never interpreted as current state. Required form is checked, legacy unstructured requirements remain diagnostic, and action replacement plus runtime-stat, Motion Value, damage-type replacement, fixed-Crit, selectors, predicates, Sequence gates, and standard modifiers pass through the shared universal action pipeline before Damage Engine formulas.

The `/team-dps` Team Rotation Validation Lab reads Character Box builds and authors only Action, Switch, and Wait steps. Intro actions are hidden from manual authoring and derived by semantic switch behavior. Unknown duration stays Missing unless the user supplies an audit-visible manual sequential duration. Versioned SSR-safe storage preserves build references, stable slot actor IDs, starting actor, steps, and overrides; stale references remain visible diagnostics. Results show raw resolved damage explicitly labelled as not final Team DPS. Repeated-cycle and authoritative Team DPS validation remain deferred to the final task.
