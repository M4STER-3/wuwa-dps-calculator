...
import { getConfiguredBuildOptions } from "./build-options";
...
describe("cohérence du catalogue", () => {
  ...
  it("intègre Aemeath comme donnée réelle et conserve les fixtures restantes explicites", () => {
    ...
    expect(aemeath.resonanceChain.map((chain) => chain.sequence)).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
    ...
    const fixtureEntries = [resonators, weapons, sonatas, mainEchoes, presets]
      ...
      .filter(
        (entry) =>
          ...
          entry.id.startsWith("preset-fixture-"),
      );
  ...
  it("fournit le preset S0 Lv90 complet et son équipement vérifié", () => {
    ...
    expect(aemeathPreset.recommendedTargets?.attack).toEqual({
      minimum: 2000,
      maximum: 2400,
    });
  ...
  it("n'expose à Aemeath que ses options réelles configurées", () => {
    const options = getConfiguredBuildOptions("aemeath");

    expect(options.weapons.map((weapon) => weapon.name)).toEqual([
      "Everbright Polestar",
    ]);
    expect(options.sonatas.map((sonata) => sonata.name)).toEqual([
      "Trailblazing Star (5-piece)",
    ]);
    expect(options.mainEchoes.map((mainEcho) => mainEcho.name)).toEqual([
      "Sigillum",
    ]);
    expect(
      [...options.weapons, ...options.sonatas, ...options.mainEchoes].every(
        (entry) => entry.source.kind !== "technical-fixture",
      ),
    ).toBe(true);
  });
  ...
  it("documente le kit Lv10 et les inconnues temporelles sans timeline DPS", () => {
    ...
    expect(
      aemeath.combat?.actions.find((entry) => entry.id === "finale")
        ?.multipliers,
    ).toEqual([{ percent: 1789.29, hits: 1 }]);
    expect(
      aemeath.combat?.effects.find((entry) => entry.id === "starburst-icd")
        ?.internalCooldown,
    ).toEqual({ seconds: 8, scope: "target" });
    expect(
      aemeath.combat?.actions.every(
        (entry) => entry.castDurationSeconds.confidence === "unknown",
      ),
    ).toBe(true);
