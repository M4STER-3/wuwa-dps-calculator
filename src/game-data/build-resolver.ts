...
export const RESONATOR_PANEL_BASELINE_V1 = Object.freeze({
  critRate: 5,
  critDamage: 150,
  energyRegen: 100,
  healingBonus: 0,
...
function exactProgressionValue(
  ...
  if (!progression || progression.interpolation !== "none") fail(`${label} has no reviewed non-interpolated progression`);
  if (!Number.isInteger(level) || level < 1 || level > 90) fail(`${label} requested invalid level ${level}`);
  ...
  if (typeof ascended !== "boolean") fail(`${label} level ${level} requires an explicit pre/post ascension choice`);
  const selected = matches.filter((point) => point.ascended === ascended);
  if (selected.length !== 1) fail(`${label} level ${level} has an invalid reviewed ascension mapping`);
  return finite(selected[0]!.value, `${label} level ${level}`);
}

function exactCharacterBase(character: CharacterCatalogEntry, level: number, ascended: boolean | undefined) {
  return {
...
function applyWeaponSecondary(
  ...
  switch (stat) {
    case "ATK": basePercent.attack += value; return;
    case "HP": basePercent.hp += value; return;
    case "DEF": basePercent.defense += value; return;
    case "Crit. Rate": statSheet.critRate += value; return;
    case "Crit. DMG": statSheet.critDamage += value; return;
    case "Energy Regen": statSheet.energyRegen += value; return;
    default: fail(`weapon secondary stat ${JSON.stringify(stat)} has no reviewed V1 semantic mapping`);
  }
}
...
function blankStatSheet(): FinalStats {
  return {
    ...
    attack: 0,
    defense: 0,
    critRate: RESONATOR_PANEL_BASELINE_V1.critRate,
    critDamage: RESONATOR_PANEL_BASELINE_V1.critDamage,
    energyRegen: RESONATOR_PANEL_BASELINE_V1.energyRegen,
    healingBonus: 0,
    tuneBreakBoost: 0,
  ...
  for (const sonata of sonataSets) {
    ...
    for (const bonus of sonata.bonuses) {
      if (equippedPieces >= bonus.pieces) {
        unresolved.push({ kind: "sonata-bonus", sonataSetId: sonata.id, pieces: bonus.pieces, description: bonus.description });
      }
    }
  }
  return unresolved;
...
export function resolveExactBuildStatSheetV1(
  ...
  const weaponAttack = exactProgressionValue(weapon.baseStats.attack, input.weaponLevel, input.weaponAscended, `${weapon.id}.attack`);
  const echoResolution = resolveEchoLoadoutV1({ echoes: database.echoes, sonataSets: database.sonataSets }, input.echoLoadout);
  ...
  if (weapon.baseStats.secondaryStat) {
    if (weapon.baseStats.secondaryStat.unit !== "percentage-points") fail(`${weapon.id} secondary stat has an unsupported unit`);
  ...
  statSheet.hp = characterBase.hp * (1 + basePercent.hp / 100) + echoResolution.contributions.flat.hp;
  statSheet.attack = (characterBase.attack + weaponAttack) * (1 + basePercent.attack / 100) + echoResolution.contributions.flat.attack;
  statSheet.defense = characterBase.defense * (1 + basePercent.defense / 100) + echoResolution.contributions.flat.defense;
  ...
  const unresolvedPermanentSources = collectUnresolvedSources(character, weapon, database.sonataSets, echoResolution.sonataPieceCounts);

  return {
    statSheet,
    complete: unresolvedPermanentSources.length === 0,
    coveredPermanentSources: ["character-base-stats", "weapon-base-stats", "weapon-secondary-stat", "echo-main-stats", "echo-substats"],
    unresolvedPermanentSources,
    echoResolution,
    exactBase: {
      character: characterBase,
      weapon: { attack: weaponAttack, ...(secondaryStat ? { secondaryStat } : {}) },
    },
  };
}
