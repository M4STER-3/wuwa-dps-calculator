...
export type NegativeStatusDamageKind = "fusionBurst";
...
export interface NegativeStatusDamageRequest {
  ...
  target: DamageTarget;
  stacks: number;
  /** Additive multiplier increase: 200 means ×(1 + 2.00). */
...
/** Fusion Burst motion values in NS-style basis points. */
 ...
 * formula. Character kits alter it through explicit stack/multiplier/amp/crit
...
export function calculateNegativeStatusDamage(
  ...
  const motionValueBasisPoints =
    request.kind === "fusionBurst" ? fusionBurstMotionValue(request.stacks) : undefined;
  if (motionValueBasisPoints === undefined) {
    throw new Error(
