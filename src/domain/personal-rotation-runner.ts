...
function scenarioRotationTarget(
  ...
  if (scenarioTarget) {
    if (!Number.isFinite(scenarioTarget.seconds) || scenarioTarget.seconds <= 0) {
      throw new Error(
        `Rotation scenario ${scenario.id} has invalid target duration ${scenarioTarget.seconds}.`,
      );
    }
...
function resourceCapForSequence(
  ...
  if (!Number.isFinite(cap) || cap <= 0) {
    throw new Error(
      `Resource ${resource.id} has invalid cap ${String(cap)} at S${sequence}.`,
    );
  }
  return cap;
}
  ...
  const resources = Object.fromEntries(
    (resonator.combat?.resources ?? []).map((resource) => {
      ...
      if (!Number.isFinite(current) || current < 0 || current > cap) {
        throw new Error(
          `Rotation scenario ${scenario.id} has invalid initial ${resource.id}: ${current}/${cap} at S${sequence}.`,
        );
      }
  ...
  return {
    ...
    perSource: {
      ...
      [resonatorId]: addAmounts(
        simulation.perSource[resonatorId] ?? {
          nonCrit: 0,
          crit: 0,
          expected: 0,
        },
...
export function runTheoreticalPersonalRotation(
  ...
  const timeline = buildTheoreticalRotationTimeline(
    ...
    actions,
    scenarioRotationTarget(
      request.scenario,
      request.resonator,
      request.build.sequence,
    ),
  );
  const externalEvents = compilePersonalScenarioEvents({
    scenario: request.scenario,
    timeline,
    sequence: request.build.sequence,
    actorId: request.resonator.id,
    targetId,
  });
  ...
  const rawSimulation = simulatePersonalCombat({
    ...
    loadout: {
      weapon: materializeWeaponForRank(
        request.weapon,
        request.build.weapon.rank,
      ),
