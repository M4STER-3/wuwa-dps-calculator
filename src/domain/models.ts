...
export interface RecommendedBuildPreset {
  ...
  progression?: {
    inherentSkillsUnlocked: boolean;
    minorFortesUnlocked: boolean;
  };
  ...
  recommendedTargets?: Readonly<
    Partial<
      Record<
        keyof Omit<FinalStats, "elementalDamageBonus" | "damageTypeBonus">,
        {
          minimum: number;
          maximum?: number;
        }
      >
    > & {
      elementalDamageBonus?: Partial<
        Record<Element, { minimum: number; maximum?: number }>
      >;
    }
  >;
  sonataId?: string;
