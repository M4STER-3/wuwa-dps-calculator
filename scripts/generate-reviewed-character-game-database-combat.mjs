...
function fail(message) {
  throw new Error(`Reviewed character GameDatabase combat projection: ${message}`);
}
...
for (const review of reviewedCharacters) {
  ...
  const characterMatches = database.characters.filter((raw) => {
    ...
    return externalIds && typeof externalIds === "object" && !Array.isArray(externalIds) && externalIds.wuwa === review.wuwaId;
  });
  if (characterMatches.length !== 1) fail(`${review.name} Wuwa ID ${review.wuwaId} resolves to ${characterMatches.length} characters`);
  const character = record(characterMatches[0], `${review.name} character`);
  if (character.name !== review.name) fail(`Wuwa ID ${review.wuwaId} identity mismatch: ${JSON.stringify(character.name)}`);
  if (!Array.isArray(character.skills)) fail(`${review.name} skills must be an array`);

  const sourceRows = [];
  const rowIds = new Set();
  for (const [skillIndex, rawSkill] of character.skills.entries()) {
    const skill = record(rawSkill, `${review.name}.skills[${skillIndex}]`);
    const sourceParameters = record(skill.sourceParameters, `${review.name}.skills[${skillIndex}].sourceParameters`);
    const sourceType = text(sourceParameters.type, `${review.name}.skills[${skillIndex}].type`, 120);
    ...
    for (const [attributeIndex, rawAttribute] of sourceParameters.attributes.entries()) {
      const attribute = record(rawAttribute, `${review.name}.skills[${skillIndex}].attributes[${attributeIndex}]`);
      const name = text(attribute.name, `${review.name}.attribute[${attributeIndex}].name`);
      if (!isDamageAttribute(name) || !Array.isArray(attribute.values) || attribute.values.length < 10) continue;
      const sourceAttributeId = text(attribute.sourceAttributeId, `${review.name}.${name}.sourceAttributeId`, 160);
      if (!/^[A-Za-z0-9._:-]+$/.test(sourceAttributeId)) fail(`${review.name}.${name} has invalid source attribute id`);
      const parsedByLevel = [];
      let scalingAttribute;
      let supported = true;
      for (let level = 1; level <= 10; level += 1) {
        const parsed = parseFormula(attribute.values[level - 1], `${review.name}.${name}.Lv${level}`);
      ...
      if (rowIds.has(sourceAttributeId)) fail(`${review.name} duplicate sourceAttributeId ${sourceAttributeId}`);
      rowIds.add(sourceAttributeId);
      sourceRows.push({
        sourceAttributeId,
        sourceSkillId: text(sourceParameters.sourceSkillId, `${review.name}.${name}.sourceSkillId`, 160),
        sourceSkillName: text(skill.name, `${review.name}.${name}.sourceSkillName`, 200),
  ...
  if (sourceRows.length !== review.expectedDamageRows) {
    fail(`${review.name} expected ${review.expectedDamageRows} exact damage rows, received ${sourceRows.length}`);
  }
  ...
  if (new Set(reviewedSourceIds).size !== reviewedSourceIds.length || reviewedSourceIds.length !== sourceRows.length) {
    fail(`${review.name} reviewed sourceAttributeId coverage is not one-to-one with projected rows`);
  }
  for (const row of sourceRows) {
    if (!reviewedSourceIds.includes(row.sourceAttributeId)) {
      fail(`${review.name} unreviewed projected row ${row.sourceAttributeId}: ${row.sourceSkillId} ${row.sourceSkillName} / ${row.name} / Lv10=${JSON.stringify(row.multipliersByTalentLevel["10"] ?? [])}`);
    }
  }
  ...
  const mappedActions = review.mappings.map((mapping) => {
    const rows = mapping.sourceAttributeIds.map((sourceAttributeId) => {
      ...
      if (matches.length !== 1) fail(`${review.name}.${mapping.actionId} sourceAttributeId ${sourceAttributeId} resolves to ${matches.length} rows`);
      const row = matches[0];
      if (row.talent !== mapping.talent) fail(`${review.name}.${mapping.actionId} talent mismatch at ${sourceAttributeId}: ${row.talent}`);
      return row;
    });
    const level10 = rows.flatMap((row) => row.multipliersByTalentLevel["10"] ?? []);
    if (!sameGroups(level10, mapping.level10)) {
      fail(`${review.name}.${mapping.actionId} Lv10 mismatch: GameDatabase=${JSON.stringify(level10)} reviewed=${JSON.stringify(mapping.level10)}`);
    }
    const multipliersByTalentLevel = Object.fromEntries(
      Array.from({ length: 10 }, (_, index) => {
        ...
        if (groups.length !== mapping.level10.length) fail(`${review.name}.${mapping.actionId} Lv${level} group count mismatch`);
        return [level, groups];
      }),
    );
    return {
  ...
  projection[review.key] = {
    sourceItemId: review.wuwaId,
    name: review.name,
    mappedActions,
  };
}
