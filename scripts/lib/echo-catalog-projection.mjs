...
function safeEntityArray(value, label) {
...
function safeSonataIds(value, label) {
...
function safeEchoCost(value, label) {
  if (value !== 1 && value !== 3 && value !== 4) fail(`${label} must be 1, 3, or 4`);
  return value;
}

/**
 * Build-time projection of the already-promoted GameDatabase.
 * Only the tiny UI subset required by the Echo editor is retained.
 */
export function projectEchoCatalogV1(source) {
  ...
  for (const [index, raw] of echoSource.entries()) {
    ...
    for (const sonataId of sonataSetIds) {
      if (!sonataIds.has(sonataId)) fail(`Echo ${id} references unknown Sonata ${sonataId}`);
    }
